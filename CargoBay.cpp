/***************************************************************************************
 ** Program Filename: ssTwilight
 ** Author: Tom Adamcewicz
 ** Date: 3/14/16
 ** Description: This is the CargoBay class file. It defines all of the Space virtual
 ** functions and creates a CargoBay space.
 ** Input: none
 ** Output: CargoBay object
 ***************************************************************************************/

#include "CargoBay.hpp"
#include <iostream>

/********************************************************************
 ** Function: CargoBay()
 ** Description: This is the CargoBay constructor, it sets all
 ** ptrs to NULL and type to Cargo Bay and checked to 0.
 ** Parameters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/ 
CargoBay::CargoBay()
{
   nextSpace = NULL;
   lastSpace = NULL;
   leftSpace = NULL;
   rightSpace = NULL;

   player = NULL;
   type = "Cargo Bay";
   checked = 0;
}


/********************************************************************
 ** Function: setPlayer()
 ** Description: sets the player in the space.
 ** Parameters: SpaceMarine* to player.
 ** Pre-Conditions: none
 ** Post-Conditions: none
 ********************************************************************/
void CargoBay::setPlayer(SpaceMarine *playerIn)
{
   player = playerIn;
}


/********************************************************************
 ** Function: enterSpace()
 ** Description: this function is displayed when the player
 ** enters this space.
 ** Parameters: none
 ** Pre-Conditions: player moves into the Space
 ** Post-Conditions: none
 *******************************************************************/
void CargoBay::enterSpace()
{
   std::cout << "********************************" << std::endl;
   std::cout << "You have entered the " << type << std::endl;
   std::cout << "********************************" << std::endl;
}


/********************************************************************
 ** Function: leaveSpace()
 ** Description: this function is displayed when the player leaves
 ** this space.
 ** Parameters: none
 ** Pre-Conditions: player leaves a Space
 ** Post-Conditions: none
 *******************************************************************/
void CargoBay::leaveSpace()
{
   std::cout << "********************************" << std::endl;
   std::cout << "You're leaving the " << type << std::endl;
   std::cout << "********************************" << std::endl;
}


/********************************************************************
 ** Function: setPtrs()
 ** Description: sets the 4 Space pointers for the object.
 ** Parameters: 4 Space* for each connecting room.
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
void CargoBay::setPtrs(Space* next, Space* last, Space* left, Space* right)
{
   nextSpace = next;
   lastSpace = last;
   leftSpace = left;
   rightSpace = right;
}


/********************************************************************
 ** Function: setPodNum
 ** Description: this function does nothing for this class.
 *******************************************************************/ 
void CargoBay::setPodNum(int pNum)
{
   NULL;
}


/********************************************************************
 ** Function: setBridgeType()
 ** Description: not used with this class.
 *******************************************************************/
void CargoBay::setBridgeType(std::string bType)
{
   NULL;
}


/******************************************************************
 ** Function: getType()
 ** Description: returns the type and pod number.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
std::string CargoBay::getType()
{
   return type;
}



/******************************************************************
 ** Function: getNext()
 ** Description: returns the next Space type.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
std::string CargoBay::getNext()
{
   return nextSpace->getType();
}


/******************************************************************
 ** Function: getLast()
 ** Description: returns the Last Space type.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
std::string CargoBay::getLast()
{
   return lastSpace->getType();
}



/******************************************************************
 ** Function: getLeft()
 ** Description: returns the left Space type.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
std::string CargoBay::getLeft()
{
   return leftSpace->getType();
}



/******************************************************************
 ** Function: getRight()
 ** Description: returns the right Space type.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
std::string CargoBay::getRight()
{
   return rightSpace->getType();
}




/******************************************************************
 ** Function: nextPtr()
 ** Description: returns the next Space pointer.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
Space* CargoBay::nextPtr()
{
   return nextSpace;
}



/******************************************************************
 ** Function: lastPtr()
 ** Description: returns the last Space pointer.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
Space* CargoBay::lastPtr()
{
   return lastSpace;
}


/******************************************************************
 ** Function: leftPtr()
 ** Description: returns the left Space pointer.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
Space* CargoBay::leftPtr()
{
   return leftSpace;
}


/******************************************************************
 ** Function: rightPtr()
 ** Description: returns the right Space pointer.
 ** Paramters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
Space* CargoBay::rightPtr()
{
   return rightSpace;
}



/********************************************************************
 ** Function: observeSpace()
 ** Description: observes the current space, to find items, aliens
 ** and clues.
 ** Parameters: none
 ** Pre-Conditions: none
 ** Post-Conditions: none
 *******************************************************************/
void CargoBay::observeSpace()
{
   std::cout << "-----OBSERVING-----" << std::endl;
   if (checked == 0) {
      std::cout << "J1M1: It looks like there's a crate of supplies in here."; 
      std::cout << std::endl;
      std::cout << "J1M1: Maybe we can find a weapon in there.";
      std::cout << std::endl;
      std::cout << "J1M1: Should we check?" << std::endl;
   } else {
      std::cout << "J1M1: We already checked that crate." << std::endl;
      std::cout << "J1M1: Should we check it again?" << std::endl;
   }
}



/********************************************************************
 ** Function: specialInteract()
 ** Description: interacts with the Space to pick up items or fight
 ** aliens.
 ** Parameters: none
 ** Pre-Conditions: none
 ** Post-Conditions: updates checked variable
 *******************************************************************/
void CargoBay::specialInteract()
{
   std::cout << "***INTERACTING***" << std::endl; 
   if (checked == 0) {
      std::cout << "J1M1: There's a Blaster Rifle in here!" << std::endl;
      std::cout << "J1M1: That should help to fight these aliens." << std::endl;
      std::cout << "J1M1: Make sure to check your bag and use it to equip";
      std::cout << std::endl;
      player->addItem("Blaster Rifle");
      checked++;
   } else if (checked == 1) {
      std::cout << "J1M1: Looks like it's empty" << std::endl;
      checked++;
   } else if (checked == 2) {
      std::cout << "J1M1: Woah..how'd we miss that?" << std::endl;
      std::cout << "J1M1: That's the Sun Blade, it'll cut throught anything.";
      std::cout << std::endl;
      player->addItem("Sun Blade");
   } else {
      std::cout << "J1M1: It's certainly empty now." << std::endl;
   }
   std::cout << "*****************" << std::endl;
   std::cout << "-----------------" << std::endl;
}

